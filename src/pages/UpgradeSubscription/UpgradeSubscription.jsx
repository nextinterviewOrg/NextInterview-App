...
import { createSubscription, upgradeSubscription } from "../../api/subscriptionApi";
...
const UpgradeSubscription = ({ planId }) => {
    ...
    const handleSubscription = async () => {
        ...
        try {
            ...
            const response = await upgradeSubscription(User._id, planId);
            const data = response;
    ...
    return (
        <button onClick={handleSubscription} style={{ padding: "12px 24px", background: "#4CAF50", color: "#fff", border: "none", borderRadius: "6px" }} disabled={subdcribing}>
            {subdcribing ? "Subscribing..." : "Upgrade Subscription"}   
        </button>
    );
};

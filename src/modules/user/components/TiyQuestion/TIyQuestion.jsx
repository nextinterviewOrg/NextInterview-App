...
const TIyQuestion = () => {
    ...
    const handleNextQuestion = async () => {
        try {
            const response = await nextTiyQuestion(question.id); // pass both
            if (response?.success && response?.data?._id) {
                navigate(`/user/tiyQuestion/${response.data._id}`);
            } else {
    ...
    const handleTryHarderQuestion = async () => {
        try {
            const response = await getTiyHarderQuestion(id);
      ...
      return;
        ...
        try {
            ...
            if (question.type === 'mcq') {
                ...
                const optionIndex = question.options.findIndex(opt => opt === textAnswer);
            ...
            case 'coding':
                return (
                    <>
                        ...
                        <TryCodingButton onClick={() => navigate(`/user/tiycodingpage/${question.id}`)}>
                            Try Coding
                        </TryCodingButton>
                    </>
                );

...
import { getTiyHarderQuestion } from "../../../../api/userMainQuestionBankApi";
...
const TiyCodingPage = () => {
    ...
    const handleTryHarderQuestion = async () => {
        try {
            ...
            const response = await getTiyHarderQuestion(questionID);

            if (response?.success===true && response?.data?._id) {
                navigate(`/user/tiycodingpage/${response.data._id}`);
            } else {
                notification.info({ message: "No harder question found." });
                // alert('No harder question found.');
                navigate(`/user/qusnsTryitYourself/${question.module_code}/${question.topic_code}`);
            }
        } catch (error) {
